...
Second, we also add noise to the image as $P_c A P_r^T + E$ ; where $E$ represents the random nois and let's see the effect of noise for direct solving equation as following,

$$ B = P_c A P_r^T + E $$.

Consider $P_c A P_r^T = B$, we can find 
$$ A_1 = A P_r^T = \text{scipy.linalg}(P_c, B) $$
We have $A_1 = A P_r^T$, then we can solve $ P_r A^T = A_1 $
